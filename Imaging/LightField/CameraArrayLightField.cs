...
namespace PROcessing.Net.Imaging.LightField {
  ...
  public class CameraArrayLightField : ALightField {
    ...
    public CameraArrayLightField Create(Bitmap[] images) => new CameraArrayLightField(images);
    ...
    protected override UnmanagedImage GetImage(int i) => Images[i];
    ...
    public new UnmanagedImage GetUnmanagedImage(int x, int y) =>
      IsInBounds(x, y) ? Images[(y * ViewColumns) + x].Clone() : null; // make copy
